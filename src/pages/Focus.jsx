...
export default function Focus() {
  ...
  const handleFinish = useCallback(() => {
    ...
    if (Notification?.permission === 'granted') {
      ...
      new Notification(t('notifTitle'), {
        body: lbl ? t('notifWith', lbl) : t('notifDone'),
        icon: '/vite.svg', tag: 'wemmer-timer',
      })
    }
    const record = db.sessions.insert({
      task_id: null,
      task_title:       labelRef.current || t('unspecified'),
  ...
  function setPreset(h, m, s) {
    const t2 = h * 3600 + m * 60 + s
    setHrs(h); setMins(m); setSecs(s)
    setTotal(t2); setRemaining(t2)
  }
  ...
  function handleMainBtn() {
    ...
    const t2 = hrs * 3600 + mins * 60 + secs
    if (t2 === 0) return
    totalRef.current = t2
  ...
  function reset() {
    ...
    const t2 = hrs * 3600 + mins * 60 + secs || 25 * 60
    setTotal(t2); setRemaining(t2)
  }
  ...
  const PRESETS = [
    { label: `5 ${t('minUnit')}`,    h: 0, m: 5,  s: 0 },
    { label: `10 ${t('minUnit')}`,   h: 0, m: 10, s: 0 },
    { label: `25 ${t('minUnit')}`,   h: 0, m: 25, s: 0 },
    { label: `45 ${t('minUnit')}`,   h: 0, m: 45, s: 0 },
    { label: `1 ${t('hrUnit')}`,     h: 1, m: 0,  s: 0 },
  ]
    ...
    <div className="animate-in">
      <div className="page-header">
        ...
        <button className="btn-icon" onClick={toggleNotif}
          title={notifOk ? (lang === 'th' ? 'ปิดการแจ้งเตือน' : 'Disable notifications') : (lang === 'th' ? 'เปิดการแจ้งเตือน' : 'Enable notifications')}>
          {notifOk
      ...
      <div className="focus-page">
        ...
        {isSetup && (
          <div style={{ width: '100%', maxWidth: 400 }}>
            <input className="input" placeholder={t('timerPH')}
        ...
        {(running || isPaused) && labelRef.current && (
          <div style={{ fontSize: 14, color: 'var(--text-2)', fontWeight: 500 }}>
            {t('workingOn')} <span style={{ color: 'var(--accent)' }}>{labelRef.current}</span>
          </div>
        )}
        ...
        {isSetup && (
          <div className="time-digits">
            <div className="time-digit-col">
              ...
              <span className="time-digit-label">{t('hUnit')}</span>
            </div>
            ...
            <div className="time-digit-col">
              ...
              <span className="time-digit-label">{t('mUnit')}</span>
            </div>
            <span className="time-digit-sep">:</span>
            <div className="time-digit-col">
              ...
              <span className="time-digit-label">{t('sUnit')}</span>
            </div>
          </div>
        )}
        ...
        {isSetup && (
          <div style={{ display: 'flex', gap: 8, flexWrap: 'wrap', justifyContent: 'center' }}>
            {PRESETS.map(p => (
              <button key={p.label} className="btn btn-ghost" style={{ fontSize: 13, padding: '6px 14px' }}
        ...
        <div className="timer-ring">
          <svg width="260" height="260" viewBox="0 0 260 260">
            <circle cx="130" cy="130" r={radius} fill="none" stroke="var(--bg-3)" strokeWidth="12" />
            <circle cx="130" cy="130" r={radius} fill="none"
              stroke={finished ? 'var(--green)' : 'var(--accent)'}
              strokeWidth="12" strokeLinecap="round"
              strokeDasharray={circ} strokeDashoffset={circ - dashOffset}
              style={{
          ...
          <div className="timer-text">
            {finished ? (
              ...
              <>
                ...
                {isPaused && (
                  <div className="timer-mode" style={{ color: 'var(--orange)' }}>{t('pausedLabel')}</div>
                )}
              </>
            )}
          </div>
                  ...
                  <div key={s.id} className="card"
                    ...
                    <div style={{ flex: 1 }}>
                      <div style={{ fontSize: 14, fontWeight: 500, color: 'var(--text)' }}>
                        {s.task_title ?? t('unspecified')}
                      </div>
                      <div style={{ fontSize: 12, color: 'var(--text-3)', marginTop: 2 }}>
                        {dM}:{dS} • {format(new Date(s.completed_at), lang === 'th' ? 'HH:mm น.' : 'HH:mm', { locale: dateLocale })}
                      </div>
                    </div>
                  </div>
                )
